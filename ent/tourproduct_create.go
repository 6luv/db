...
func (tpc *TourProductCreate) Save(ctx context.Context) (*TourProduct, error) {
	...
	if len(tpc.hooks) == 0 {
		...
		nv, ok := v.(*TourProduct)
		if !ok {
			return nil, fmt.Errorf("unexpected node type %T returned from TourProductMutation", v)
		}
		node = nv
	}
	return node, err
...
func (tpc *TourProductCreate) check() error {
	...
	if _, ok := tpc.mutation.Price(); !ok {
		return &ValidationError{Name: "price", err: errors.New(`ent: missing required field "TourProduct.price"`)}
	}
	if _, ok := tpc.mutation.ForSale(); !ok {
		return &ValidationError{Name: "forSale", err: errors.New(`ent: missing required field "TourProduct.forSale"`)}
	}
	if _, ok := tpc.mutation.ManagerID(); !ok {
		return &ValidationError{Name: "manager", err: errors.New(`ent: missing required edge "TourProduct.manager"`)}
	}
	return nil
}
...
func (tpc *TourProductCreate) createSpec() (*TourProduct, *sqlgraph.CreateSpec) {
	...
	if value, ok := tpc.mutation.Price(); ok {
		_spec.SetField(tourproduct.FieldPrice, field.TypeInt, value)
		_node.Price = value
	}
	if value, ok := tpc.mutation.ForSale(); ok {
		_spec.SetField(tourproduct.FieldForSale, field.TypeBool, value)
		_node.ForSale = value
	}
